/*
 * Copyright (c) 2014-2015 by Wen Yu. All rights reserved. This program and the
 ...
 * equal zero, it will be determined from the first frame
...
private void rehash() {
  ...
  for (HashEntry<E> oldArray1 : oldArray) {
    if (oldArray1 != null && oldArray1.isActive) {
      put(oldArray1.key, oldArray1.value);
    }
  }
}
...
public void createInverseMap(int no_of_colors, int[] colorPalette) {
  ...
  for (int i = 0; i < no_of_colors; i++) {
     ...
     * color map entry as follows: (rcenter * x + x/2) - red, where rcenter is
     * the center of the Quantized red color map entry which is 0 since we start
     * from 0.
     */
